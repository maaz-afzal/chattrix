...
const ChatArea = ({ selected }) => {
  const [refreshMessage, setRefreshMessage] = useState(0);
  ...
  return (
    <main className="flex-1 min-w-0 bg-gray-800/60 backdrop-blur-xl rounded-3xl shadow-xl border border-gray-700/40 flex flex-col overflow-hidden">
      ...
      <MessageList selected={selected} refreshTrigger={refreshMessage} />
      <MessageInput selected={selected} onMsgSent={handleMessageSent} />
    </main>
  );
};
